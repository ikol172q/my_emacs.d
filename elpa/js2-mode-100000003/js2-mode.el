...
(defcustom js2-ignored-prop-left-names
  '("state"
    "factory"
    "config"
    "service"
    "module"
    "controller")
  "Stop get left of `js2-prop-get-node' if `js2-call-node' name is in the list.
User may want to show PART of Method Chain."
  :type 'list
  :group 'js2-mode)
;; scanner variables
...
(defun js2-format-call-node-args (args)
  "Format ARGS from JS function to string."
  (mapconcat (lambda (arg)
               (cond
                ((js2-number-node-p arg)
                 (format "%d" (js2-number-node-num-value arg)))
                ((js2-string-node-p arg)
                 (js2-node-string arg))
                ((js2-function-node-p arg)
                 "fn{..}")
                (t
                 "{}")))
             args
...
(defun js2-call-node-output-for-compute-nesed-prop (node &optional extra-args)
  "Format NODE and EXTRA-ARGS to string.  NODE corresponds to the JS function name and EXTRA-ARGS to parameters."
  (list (if (and extra-args (js2-name-node-p node))
            (format "%s(%s)"
                    (js2-name-node-name node)
                    (js2-format-call-node-args extra-args))
          node)))

(defun js2-compute-nested-prop-get (node &optional extra-args)
...
If NODE is `js2-call-node', EXTRA-ARGS is NOT nil."
  (let (left right head)
    (cond
     ((or (js2-name-node-p node)
          (js2-this-or-super-node-p node))
      (js2-call-node-output-for-compute-nesed-prop node extra-args))
     ;; For JS code `baz.setTimeout(function() { ... }, 1000);`:
     ;;  `setTimeout(function() { ... }, 1000)` is the `js2-call-node'
     ;;  `setTimeout` is the name of target of `js2-call-node'
     ((js2-call-node-p node)        ; foo.bar
      (js2-compute-nested-prop-get (js2-call-node-target node)
                                   (js2-call-node-args node)))
     ...
     ((js2-prop-get-node-p node)        ; foo.bar
      ...
      ;; For JS code like `foo.state(...)`':
      ;; Rule 1, if extra-args is nil, show "foo.state(...)".
      ;; Rule 2, Show "state(...)" if "state" is among `js2-ignored-prop-left-names'.
      ;; "state" is the `js2-name-node', "(...)" is the `extra-args'.
      (nconc (if (or (not extra-args)
                     (and (js2-name-node-p right)
                          (not (member (js2-name-node-name right)
                                       js2-ignored-prop-left-names))))
                 (setq head (js2-compute-nested-prop-get left)))
             (js2-call-node-output-for-compute-nesed-prop right
                                                          extra-args)))
